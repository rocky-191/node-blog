...
router.get("/captcha", async ctx => {
    console.log('ctx.session.captcha',ctx.session.captcha);//生成的验证码，用于比对用户是否输入正确
    const { token, buffer } = await captcha({ size: 4 });//4位图形验证码
...
router.get("/sms",async ctx=>{
    ...
    //https://sms.miaodiyun.com/register.html，短信服务提供商
